...
async function fetchDirection(start, end) {
  let origin = `${start.lat},${start.lng}`;
  let destination = `${end.lat},${end.lng}`;
  //fetching A->B route obj:
  let resRoute = await fetchRequestHelper(DIRECTIONS_API_URL + `origin=${origin}&destination=${destination}&key=${anyApiKey}`);
  return resRoute;
}
...
async function fetchDistanceMatrix(start, end) {
  let origin = `${start.lat},${start.lng}`;
  let destination = `${end.lat},${end.lng}`;
...
async function fetchNearbyPlaces(location, radius, type) {
  ...
  return fetchRequestHelper(PLACES_API + `location=${formattedLocation}&radius=500&type=bar&key=${anyApiKey}`);
}
